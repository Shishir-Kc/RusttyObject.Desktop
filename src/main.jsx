...
async function api(path, options = {}) {
  ...
  try {
    ...
    throw new Error('Could not connect to RusttyObject. Make sure the backend is running and try again.')
  }
...
function CreateBucketModal({ repository, onClose, onCreated }) {
  ...
  const submit = async (event) => {
    ...
    if (!trimmed) return setError('Enter a bucket name.')
    setBusy(true); setError('')
    try {
      await api(`/api/repositories/${encodeURIComponent(repository.owner.login)}/${encodeURIComponent(repository.name)}/buckets`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: trimmed, branch: repository.default_branch }),
      })
      await onCreated(trimmed)
      onClose()
    } catch (createError) { setError(createError.message) } finally { setBusy(false) }
  }
  return <div className="modal-backdrop" onClick={onClose}><form className="upload-modal" onClick={(event) => event.stopPropagation()} onSubmit={submit}><div className="modal-head"><div><div className="modal-icon"><Icon name="box" size={20} /></div><h2>New bucket</h2><p>Create an empty directory in <strong>{repository.full_name}</strong>.</p></div><button type="button" className="modal-close" onClick={onClose}><Icon name="close" size={18} /></button></div><label className="modal-label">Bucket name<input className="modal-input" autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. images" maxLength={100} /><small>Letters, numbers, spaces, hyphens, and underscores only.</small></label>{error && <div className="error-banner">{error}</div>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy}><Icon name="plus" size={16} /> {busy ? 'Creating…' : 'Create bucket'}</button></div></form></div>
}
...
function BucketsPage({ buckets, objects, openUpload, openCreateBucket, onPreview }) {
